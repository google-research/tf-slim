...
class _RegressionTargetColumn(_TargetColumn):
  ...
  def get_eval_ops(self, features, logits, labels, metrics=None):
    ...
    result = {"loss": metrics_lib.mean(loss)}
    if metrics:
...
class _MultiClassTargetColumn(_TargetColumn):
  ...
  def get_eval_ops(self, features, logits, labels, metrics=None):
    ...
    if metrics is None:
      ...
      metrics = {("accuracy", "classes"): _streaming_accuracy}
...
class _BinarySvmTargetColumn(_MultiClassTargetColumn):
  ...
  def __init__(self, label_name, weight_column_name):
    ...
    def loss_fn(logits, target):
      ...
      return loss_ops.hinge_loss(target, logits,
                                 reduction=loss_ops.Reduction.NONE)
...
def get_default_binary_metrics_for_eval(thresholds):
  ...
  for threshold in thresholds:
    ...
    metrics[_MetricKeys.PRECISION_MEAN % threshold] = _streaming_at_threshold(
        _streaming_precision_at_thresholds, threshold)
    # Recall for positive examples.
    metrics[_MetricKeys.RECALL_MEAN % threshold] = _streaming_at_threshold(
        _streaming_recall_at_thresholds, threshold)
...
def _streaming_auc(predictions, labels, weights=None):
    return metrics_lib.auc(predictions=predictions, labels=labels,
...
def _streaming_precision_at_thresholds(predictions, labels, thresholds,
                                      weights=None,
                                      metrics_collections=None,
                                      updates_collections=None, name=None):
  return metrics_lib.precision_at_thresholds(
...
def _streaming_recall_at_thresholds(predictions, labels, thresholds,
                                   weights=None, metrics_collections=None,
                                   updates_collections=None, name=None):
  return metrics_lib.recall_at_thresholds(
      thresholds=thresholds,
      predictions=predictions, labels=labels, weights=weights,
      metrics_collections=metrics_collections,
      updates_collections=updates_collections, name=name)
